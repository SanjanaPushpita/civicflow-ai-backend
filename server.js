...
function includesAny(text, words) {
  return words.some((word) =>
    text.includes(String(word || "").toLowerCase().trim())
  );
}
...
function isDashboardAuthorized(req) {
  return (
    ...
    safeCompare(getCookie(req, ADMIN_COOKIE_NAME), makeAdminSessionToken())
  );
}
...
function getSafeHelplineRoute(category, transcript) {
  ...
  if (includesAny(text, ["kidnap", "kidnapped", "abduction", "missing child"])) {
    return {
      isEmergency: true,
      emergencyType: "Kidnapping / Abduction Emergency",
      helpline: helplines.nationalEmergency,
  ...
  if (
    ...
    return {
      isEmergency: true,
      emergencyType: "Women / Child Safety Emergency",
  ...
  if (
    ...
    includesAny(text, ["danger", "missing", "abuse", "violence", "kidnap"])
  ) {
    return {
      isEmergency: true,
      emergencyType: "Child Safety Emergency",
      helpline: helplines.childHelpline,
      secondaryHelpline: helplines.nationalEmergency,
      liveTrackingRequired: true,
    };
  }
  ...
  if (
    ...
    return {
      isEmergency: true,
      emergencyType: "Disaster Emergency",
      helpline: helplines.nationalEmergency,
      secondaryHelpline: helplines.disasterWarning,
      liveTrackingRequired: true,
    };
  }
...
async function analyzeWithGemini(transcript) {
  ...
  return repairReport(
    safeJsonParse(await callGemini([{ text: prompt }])),
    transcript
  );
}
...
async function analyzeAudioWithGemini(audioBase64, mimeType) {
  const prompt =
    "You are CivicFlow AI for Bangladesh. Transcribe short audio in Bangla, English, Banglish, or regional Bangla. Return ONLY JSON: detectedLanguage, originalTranscript, banglaTranscript, englishTranslation, banglishRoman, transcriptionConfidence, needsRepeat, repeatReason, report{intent,category,location,summary,recommendedAction,confidence}. Do not say unclear unless almost no human voice. Emergency/bipod/help/fire/ambulance/kidnap/harassment/disaster must be routed as emergency.";
  ...
  const allText = normalize(
    `${original} ${parsed.banglaTranscript || ""} ${
      parsed.englishTranslation || ""
    } ${parsed.banglishRoman || ""} ${parsed.report?.category || ""} ${
...
function analyzeWithRules(transcript) {
  ...
  if (includesAny(text, ["fire", "smoke", "burning"])) {
  ...
  } else if (includesAny(text, ["kidnap", "kidnapped", "missing", "abduction"])) {
    category = "Kidnapping / Abduction Emergency";
  } else if (includesAny(text, ["ambulance", "accident", "injured", "blood"])) {
    category = "Medical / Ambulance Emergency";
  } else if (includesAny(text, ["woman", "harassment", "abuse", "violence"])) {
    category = "Women / Child Safety Emergency";
  } else if (
    includesAny(text, [
      "police",
      "theft",
    ...
    category = "Police / Safety Emergency";
  } else if (includesAny(text, ["cyclone", "flood", "earthquake", "disaster"])) {
...
async function analyzeTranscript(transcript) {
  try {
    return {
      mode: "gemini",
      report: await analyzeWithGemini(transcript),
    };
  } catch (error) {
    return {
      mode: "rules-fallback",
      geminiError: cleanAiErrorMessage(error.message),
      report: analyzeWithRules(transcript),
    };
  }
}
...
function sanitizeLocationData(locationData) {
  ...
  return {
    ...
    capturedAtIso:
      locationData.capturedAtIso ||
      locationData.updatedAtIso ||
...
function mergeLocationIntoReport(report, locationData) {
  ...
  return {
    ...
    gpsCapturedAtIso: loc.capturedAtIso,
    mapsUrl: loc.mapsUrl,
  };
}
...
async function saveReport(savedReport) {
  ...
  if (!firestoreDb) {
    return { storage: "memory", firebaseStatus, firebaseError };
  }
...
async function updateReportStatus(reportId, status, adminNote) {
  ...
  if (index !== -1) {
    memoryReports[index] = { ...memoryReports[index], ...updatePayload };
  }
...
function getLiveSessionComputedStatus(session) {
  ...
  return status === "active" && expiresAtMs && Date.now() > expiresAtMs
    ? "expired"
    : status;
}
...
async function updateReportLiveFields(session) {
  ...
  if (index !== -1) {
    memoryReports[index] = { ...memoryReports[index], ...payload };
  }

  if (!firestoreDb || !session.reportId) {
    return;
  }
...
async function loadLiveSessionById(sessionId) {
  ...
  if (memoryLiveSessions.has(id)) {
    return memoryLiveSessions.get(id);
  }
...
async function handleLiveTrackingStart(req, res) {
  try {
    ...
    const durationMinutes = Number.isFinite(durationRaw)
      ? Math.min(Math.max(durationRaw, 1), 120)
      : 30;
    const firstLocation = sanitizeLocationData(body.initialLocation);

    if (!reportId) {
      return sendJson(res, 400, { ok: false, error: "Missing reportId." });
    }

    if (!firstLocation) {
      return sendJson(res, 400, {
        ok: false,
        error: "Missing valid initialLocation for live tracking.",
      });
    }
    ...
    sendJson(res, 201, {
      ok: true,
      message: "Emergency live tracking started.",
      liveSession,
      ...storageResult,
    });
  } catch (error) {
...
async function handleLiveTrackingUpdate(req, res) {
  try {
    ...
    if (!cleanLocation) {
      return sendJson(res, 400, {
        ok: false,
        error: "Missing valid locationData.",
      });
    }
    ...
    if (!existing) {
      return sendJson(res, 404, {
        ok: false,
        error: "Live tracking session not found.",
      });
    }
    ...
    const updated = {
      ...
      lastSequence: Number.isFinite(sequence)
        ? sequence
    ...
    sendJson(res, 200, {
      ...
      message:
        updated.status === "expired"
          ? "Live tracking session has expired. Last location saved."
...
async function handleLiveTrackingList(req, res) {
  ...
  sendJson(res, 200, {
    ...
    storage: firestoreDb ? "firebase-firestore" : "memory",
    firebaseStatus,
    firebaseError,
  });
}
...
async function handleAnalyzeText(req, res) {
  try {
    ...
    if (!body.transcript || typeof body.transcript !== "string") {
      return sendJson(res, 400, { ok: false, error: "Missing transcript" });
    }

    sendJson(res, 200, {
      ok: true,
      ...(await analyzeTranscript(body.transcript)),
    });
  } catch (error) {
    sendJson(res, 500, {
      ok: false,
...
async function handleAnalyzeAudio(req, res) {
  try {
    ...
    if (!body.audioBase64 || typeof body.audioBase64 !== "string") {
      return sendJson(res, 400, { ok: false, error: "Missing audioBase64" });
    }
...
async function handleSubmitReport(req, res) {
  try {
    ...
    if (!body.report || typeof body.report !== "object") {
      return sendJson(res, 400, { ok: false, error: "Missing report object" });
    }
...
body{margin:0;min-height:100vh;display:grid;place-items:center;background:#070b10;color:#f8fafc;font-family:Arial;padding:20px}
.card{width:100%;max-width:460px;background:#0f1722;border:1px solid rgba(56,189,248,.25);border-radius:28px;padding:28px}
p{color:#94a3b8;line-height:1.5}
input{width:100%;padding:14px;border-radius:16px;border:1px solid rgba(148,163,184,.24);background:#111827;color:#fff}
button{width:100%;margin-top:16px;padding:14px;border:0;border-radius:999px;background:#38bdf8;color:#020617;font-weight:900}
.warning,.error{padding:13px;border-radius:16px;margin-bottom:14px;font-weight:800}
.warning{background:rgba(251,191,36,.12);color:#fbbf24}
.error{background:rgba(248,113,113,.12);color:#fb7185}
</style>
</head>
<body>
<main class="card">
<h1>CivicFlow AI</h1>
<p>Protected admin dashboard for reports and emergency live tracking.</p>
${
  !ADMIN_PASSWORD
    ? `<div class="warning">ADMIN_PASSWORD is not set in Render Environment yet.</div>`
    : ""
}
...
async function handleDashboardLogin(req, res) {
  ...
  if (!safeCompare(password, ADMIN_PASSWORD)) {
    return sendHtml(
      res,
      buildDashboardLoginPage("Wrong password. Try again."),
      401
    );
  }
  ...
  redirect(res, "/dashboard/reports", {
    ...
    )}; HttpOnly; SameSite=Lax; Path=/; Max-Age=86400${getSecureCookieSuffix(
      req
    )}`,
  });
}
...
function handleDashboardLogout(req, res) {
  redirect(res, "/dashboard", {
    "Set-Cookie": `${ADMIN_COOKIE_NAME}=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${getSecureCookieSuffix(
      req
    )}`,
  });
}
...
async function handleUpdateReportStatus(req, res) {
  if (!isDashboardAuthorized(req)) {
    return sendHtml(res, buildDashboardLoginPage("Please login first."), 401);
  }
...
<p style="font-family:Arial;background:#070b10;padding:0 30px 30px"><a style="color:#38bdf8" href="/dashboard/reports">Back</a></p>`,
      500
    );
  }
}
...
function buildDashboardCss() {
  return `<style>
*{box-sizing:border-box}
body{margin:0;background:#070b10;color:#f8fafc;font-family:Arial}
.layout{display:grid;grid-template-columns:260px 1fr;min-height:100vh}
aside{border-right:1px solid rgba(56,189,248,.14);padding:20px;background:#081018;position:sticky;top:0;height:100vh;overflow:auto}
main{padding:26px}
h1,h2,h3{letter-spacing:-.02em}
p{color:#cbd5e1;line-height:1.45}
.pill,.badge{display:inline-flex;align-items:center;justify-content:center;padding:7px 11px;border-radius:999px;font-size:12px;font-weight:900}
.pill{background:rgba(56,189,248,.1);color:#38bdf8}
.side-card,.stat,.report,.live,.section,.mini,.gps,.text{background:#0f1722;border:1px solid rgba(148,163,184,.14);border-radius:18px}
.side-card{padding:14px;margin:10px 0}
.side-card span,.mini span,.gps span,.text span{display:block;color:#94a3b8;text-transform:uppercase;font-size:11px;letter-spacing:.12em;font-weight:900;margin-bottom:7px}
.good{background:rgba(34,197,94,.1);border-color:rgba(34,197,94,.24)}
.bad{background:rgba(248,113,113,.1);border-color:rgba(248,113,113,.24)}
.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin:22px 0}
.stat{padding:18px}
.stat b{display:block;font-size:32px;color:#38bdf8}
.red b{color:#fb7185}
.green b{color:#22c55e}
.top{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:12px}
.actions{display:flex;align-items:center;justify-content:flex-end;gap:10px;flex-wrap:wrap;flex:0 0 auto}
.actions form{margin:0;display:flex;align-items:center;flex:0 0 auto}
.button{display:inline-flex;align-items:center;justify-content:center;text-decoration:none;border:0;border-radius:999px;padding:0 16px;min-height:42px;height:42px;width:auto;max-width:max-content;line-height:1;font-weight:900;cursor:pointer;background:#38bdf8;color:#020617;white-space:nowrap;flex:0 0 auto}
.actions button.button{margin:0;width:auto}
.dark{background:#111827;color:#f8fafc;border:1px solid rgba(148,163,184,.18)}
.logout{background:#3b111a;color:#fff}
.nav-tabs{display:flex;gap:10px;flex-wrap:wrap;margin:18px 0 8px}
.nav-tab{display:inline-flex;align-items:center;justify-content:center;text-decoration:none;border-radius:999px;padding:11px 16px;font-size:13px;font-weight:900;border:1px solid rgba(148,163,184,.18);background:#0f1722;color:#cbd5e1}
.nav-tab.active{background:#38bdf8;color:#020617;border-color:#38bdf8}
.section,.report,.live{padding:18px;margin-bottom:18px}
.report.emergency{border-color:rgba(251,113,133,.28)}
.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}
.badge.normal{background:rgba(56,189,248,.12);color:#38bdf8}
.badge.emergency{background:rgba(251,113,133,.12);color:#fb7185}
.badge.live{background:rgba(34,197,94,.12);color:#22c55e}
.badge.expired{background:rgba(148,163,184,.12);color:#cbd5e1}
.badge.stopped{background:rgba(251,191,36,.12);color:#fbbf24}
.mini,.gps,.text{padding:13px;margin-top:10px}
.gps{background:#102033;border-color:rgba(56,189,248,.24)}
.action{background:rgba(34,197,94,.08)}
select,textarea{width:100%;margin-top:8px;border-radius:12px;border:1px solid rgba(148,163,184,.18);background:#081018;color:#fff;padding:12px}
textarea{min-height:70px}
.empty{padding:24px;border:1px dashed rgba(56,189,248,.25);border-radius:20px;text-align:center;color:#cbd5e1}
.notice{padding:14px;border-radius:18px;background:rgba(56,189,248,.09);border:1px solid rgba(56,189,248,.2);color:#cbd5e1;font-weight:800}
details.history{margin-top:18px}
details.history summary{cursor:pointer;color:#cbd5e1;font-weight:900;padding:14px;border-radius:18px;background:#0f1722;border:1px solid rgba(148,163,184,.14)}
@media(max-width:900px){
.layout,.stats,.grid{grid-template-columns:1fr}
...
function buildStorageBox(storage, firebaseErrorValue) {
  if (storage === "firebase-firestore" || firebaseStatus === "connected") {
    return `<div class="side-card good"><span>Storage</span><b>Firebase Firestore connected</b></div>`;
  }

  return `<div class="side-card bad"><span>Storage</span><b>${escapeHtml(
    storage || firebaseStatus
  )}<br>${escapeHtml(
    firebaseErrorValue || firebaseError || "Firebase not connected"
  )}</b></div>`;
}
...
function buildDashboardSidebar({
  ...
  return `<aside>
<span class="pill">● Protected Backend</span>
<h1>CivicFlow AI</h1>
<p>Admin dashboard for Flutter reports.</p>
<div class="side-card"><span>AI Mode</span><b>${
    GEMINI_API_KEY ? "Gemini Ready" : "Fallback Only"
  }</b></div>
<div class="side-card"><span>Model</span><b>${escapeHtml(GEMINI_MODEL)}</b></div>
${buildStorageBox(storage, firebaseErrorValue)}
<div class="side-card"><span>Latest Report</span><b>${escapeHtml(latest)}</b></div>
<div class="side-card"><span>Total Reports</span><b>${total}</b></div>
<div class="side-card bad"><span>Emergency Routes</span><b>${emergency}</b></div>
<div class="side-card"><span>Normal Routes</span><b>${normal}</b></div>
<div class="side-card good"><span>Live Tracking</span><b>${activeLive} active session(s)</b></div>
</aside>`;
}

function buildTopActions(activePage, refreshPath) {
  return `<div class="nav-tabs">
<a class="nav-tab ${activePage === "reports" ? "active" : ""}" href="/dashboard/reports">Reports</a>
<a class="nav-tab ${activePage === "live" ? "active" : ""}" href="/dashboard/live">Live Tracking</a>
</div>
<div class="actions">
<a class="button dark" href="/api/civicflow/admin/reports-json" target="_blank">View JSON</a>
...
<div class="stat"><b>${total}</b><strong>Total Reports</strong></div>
<div class="stat red"><b>${emergency}</b><strong>Emergency Routes</strong></div>
<div class="stat"><b>${normal}</b><strong>Normal Routes</strong></div>
<div class="stat green"><b>${activeLive}</b><strong>Active Live</strong></div>
</div>`;
}
...
function buildDashboardLayout({
  ...
  const metaRefresh = autoRefreshSeconds
...
<div>
${buildTopActions(activePage, refreshPath)}
</div>
</div>
${stats}
${content}
...
async function handleReportsDashboard(req, res) {
  ...
  const activeLive = liveSessions.filter(
    (x) => getLiveSessionComputedStatus(x) === "active"
  ).length;

  const latest = reports[0]
    ? `${reports[0].report?.category || "Report"} • ${formatDateTime(
        reports[0].createdAt
      )}`
    : "No report yet";
  ...
  const sidebar = buildDashboardSidebar({
    ...
    firebaseErrorValue: loadResult.firebaseError,
  });

  const cards =
    reports
      .map((item, index) => buildReportCard(item, index, liveSessions, false))
      .join("") || `<div class="empty">No reports received yet.</div>`;

  const content = `<section class="section">
<h2>Reports</h2>
<p>Review, update, and manage submitted citizen reports. Live tracking has been moved to its own page.</p>
${cards}
</section>`;
  ...
  sendHtml(
    ...
    buildDashboardLayout({
      ...
      subtitle: "Review and manage citizen help routes clearly.",
      activePage: "reports",
      refreshPath: "/dashboard/reports",
      autoRefreshSeconds: null,
      sidebar,
      stats: buildStats(total, emergency, normal, activeLive),
      content,
    })
  );
}
...
async function handleLiveDashboard(req, res) {
  ...
  const activeSessions = liveSessions.filter(
    (session) => getLiveSessionComputedStatus(session) === "active"
  );

  const inactiveSessions = liveSessions.filter(
    (session) => getLiveSessionComputedStatus(session) !== "active"
  );
  ...
  const activeCards =
    activeSessions.map(buildLiveSessionCard).join("") ||
    `<div class="empty">No active live tracking sessions right now.</div>`;

  const inactiveCards =
    inactiveSessions.slice(0, 10).map(buildLiveSessionCard).join("") ||
    `<div class="empty">No expired or stopped live sessions yet.</div>`;

  const content = `<section class="section">
<h2>Active Live Emergency Tracking</h2>
<div class="notice">This page refreshes every 10 seconds. Active sessions are shown first so authorities can quickly open the latest location.</div>
<br>
${activeCards}
</section>
<details class="history">
<summary>Show recent expired / stopped live sessions</summary>
<br>
${inactiveCards}
</details>`;
  ...
  sendHtml(
    ...
    buildDashboardLayout({
      ...
      subtitle: "Monitor active emergency live location sessions separately.",
      activePage: "live",
      refreshPath: "/dashboard/live",
...
function buildReportCard(item, index, liveSessions, showLiveMini = false) {
  ...
  const live = item.liveSessionId
    ? liveSessions.find((s) => s.id === item.liveSessionId)
    : liveSessions.find((s) => String(s.reportId) === String(item.id));
  const liveStatus = live ? getLiveSessionComputedStatus(live) : null;
  const help =
    r.helplineLabel && r.helplineNumber
      ? `${r.helplineLabel} • ${r.helplineNumber}`
      : "No helpline selected";

  return `<article class="report ${isE ? "emergency" : ""}">
<div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
<div>
<span class="badge ${isE ? "emergency" : "normal"}">${
    isE ? "Emergency" : "Normal"
  }</span>
${
  liveStatus
    ? `<span class="badge ${
        liveStatus === "active" ? "live" : liveStatus === "stopped" ? "stopped" : "expired"
      }">Live ${escapeHtml(liveStatus)}</span>`
    : ""
}
<span style="color:#94a3b8;font-weight:900">#${index + 1}</span>
<h2>${escapeHtml(r.category || "Citizen Report")}</h2>
</div>
<b>${escapeHtml(formatDateTime(item.createdAt))}</b>
</div>
<div class="grid">
<div class="mini"><span>Status</span><b>${escapeHtml(item.status || "Received")}</b></div>
<div class="mini"><span>Source</span><b>${escapeHtml(item.source || "flutter-app")}</b></div>
<div class="mini"><span>Area</span><b>${escapeHtml(r.location || "Current user area")}</b></div>
<div class="mini"><span>Help</span><b>${escapeHtml(help)}</b></div>
</div>
${buildGpsBlock(r)}
${showLiveMini && live ? buildLiveMiniBlock(live) : ""}
<div class="text"><span>Transcript</span>${escapeHtml(item.transcript || "No transcript")}</div>
<div class="text"><span>AI Summary</span>${escapeHtml(r.summary || "No summary")}</div>
<div class="text action"><span>Recommended Action</span>${escapeHtml(
    r.recommendedAction || "No action"
  )}</div>
<form method="POST" action="/dashboard/update-status" style="margin-top:14px;padding:14px;border-radius:16px;background:rgba(148,163,184,.07)">
<input type="hidden" name="reportId" value="${escapeHtml(item.id)}">
<label><b>Update Status</b></label>
<select name="status">${buildStatusOptions(item.status)}</select>
<label style="display:block;margin-top:10px"><b>Admin Note</b></label>
<textarea name="adminNote" placeholder="Write admin note or action taken...">${escapeHtml(
    item.adminNote || ""
  )}</textarea>
<button class="button" type="submit" style="margin-top:10px">Save Status</button>
</form>
<p style="font-size:12px;color:#64748b"><b>ID:</b> ${escapeHtml(item.id)}</p>
</article>`;
}
...
function buildGpsBlock(r) {
  const hasGps =
    r.gpsLatitude !== null &&
    r.gpsLatitude !== undefined &&
    r.gpsLongitude !== null &&
    r.gpsLongitude !== undefined;
  ...
  const url =
    r.mapsUrl || `https://www.google.com/maps?q=${r.gpsLatitude},${r.gpsLongitude}`;

  return `<div class="gps">
<span>GPS Location</span>
<b>Latitude: ${escapeHtml(r.gpsLatitude)}<br>Longitude: ${escapeHtml(r.gpsLongitude)}</b>
<br>
<span style="margin-top:8px;text-transform:none;letter-spacing:0;color:#cbd5e1">
GPS Accuracy: about ${escapeHtml(r.gpsAccuracyMeters || "unknown")} meters<br>
Captured: ${escapeHtml(formatDateTime(r.gpsCapturedAtIso))}
</span>
<p><a class="button dark" href="${escapeHtml(url)}" target="_blank">Open in Google Maps</a></p>
</div>`;
}
...
function buildLiveMiniBlock(s) {
  ...
  return `<div class="gps" style="background:rgba(34,197,94,.08);border-color:rgba(34,197,94,.28)">
<span>Live Tracking</span>
<b>Status: ${escapeHtml(getLiveSessionComputedStatus(s))}<br>
Updates: ${escapeHtml(s.updateCount || 0)}<br>
Expires: ${escapeHtml(formatDateTime(s.expiresAtIso))}</b>
${url ? `<p><a class="button" href="${escapeHtml(url)}" target="_blank">Open Latest Live Location</a></p>` : ""}
</div>`;
}
...
function buildLiveSessionCard(s) {
  ...
  const statusClass =
    status === "active" ? "live" : status === "stopped" ? "stopped" : "expired";

  return `<article class="live">
<span class="badge ${statusClass}">${escapeHtml(status)}</span>
<h3>Live Session ${escapeHtml(s.id)}</h3>
<div class="grid">
<div class="mini"><span>Report ID</span><b>${escapeHtml(s.reportId || "Unknown")}</b></div>
<div class="mini"><span>Started</span><b>${escapeHtml(formatDateTime(s.startedAtIso))}</b></div>
<div class="mini"><span>Expires</span><b>${escapeHtml(formatDateTime(s.expiresAtIso))}</b></div>
<div class="mini"><span>Updates</span><b>${escapeHtml(s.updateCount || 0)}</b></div>
</div>
${
  loc.latitude && loc.longitude
    ? `<div class="gps">
<span>Latest Live Location</span>
<b>Latitude: ${escapeHtml(loc.latitude)}<br>
Longitude: ${escapeHtml(loc.longitude)}<br>
GPS Accuracy: about ${escapeHtml(loc.accuracyMeters || "unknown")} meters</b>
${
  s.lastLocationAtIso
    ? `<br><span style="margin-top:8px;text-transform:none;letter-spacing:0;color:#cbd5e1">Last update: ${escapeHtml(
        formatDateTime(s.lastLocationAtIso)
      )}</span>`
    : ""
}
${url ? `<p><a class="button" href="${escapeHtml(url)}" target="_blank">Open Latest Location</a></p>` : ""}
</div>`
    : `<div class="empty">No live location point saved for this session yet.</div>`
}
${
  s.stopReason
    ? `<div class="text"><span>Stop Reason</span>${escapeHtml(s.stopReason)}</div>`
    : ""
}
</article>`;
}
...
async function routeRequest(req, res) {
  ...
  try {
    ...
    if (req.method === "POST" && ["/api/civicflow/reports", "/api/reports"].includes(path)) {
      return await handleSubmitReport(req, res);
    }

    if (req.method === "GET" && ["/api/civicflow/reports", "/api/reports"].includes(path)) {
